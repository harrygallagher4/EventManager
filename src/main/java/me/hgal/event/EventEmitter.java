...
public class EventEmitter {
    ...
    private Map<Class<? extends Event>, Consumer<? extends Event>>
     ...
     * @see Consumer#andThen
     */
    public <C extends Event> void on(Class<C> eventClass,
                                     Consumer<C> listener) {
    ...
    /**
     * Alias for {@code this.on}
     *
     * @see #on
     */
    public <C extends Event> void addListener(Class<C> eventClass,
                                              Consumer<C> listener) {
        this.on(eventClass, listener);
    }
     ...
     */
    public <C extends Event> boolean removeListener(Class<C> eventClass,
                                                    Consumer<C> listener) {
        List<Consumer<? extends Event>> listeners =
                this.allListeners.get(eventClass);
        if (listeners.remove(listener)) {
            Consumer<C> combined = this.reduce(eventClass, null,
                    listeners.stream());
            this.functions.put(eventClass, combined);
            return true;
        }
     ...
     *
     * @param eventClass    {@code Class} of {@code} Event to clear
     * @return              {@code true} if listeners existed
     */
    public boolean removeAllListeners(Class<? extends Event> eventClass) {
        if (this.functions.containsKey(eventClass) &&
                this.allListeners.containsKey(eventClass)) {
            this.functions.remove(eventClass);
            this.allListeners.remove(eventClass);
    ...
    /**
     * Returns the number of listeners listening on {@code eventClass}
     *
     * @param eventClass    {@code Class} of {@code Event} to check
     * @return              number of listeners listening on {@code eventClass}
     */
    public int listenerCount(Class<? extends Event> eventClass) {
        List<Consumer<? extends Event>> listeners =
                this.allListeners.get(eventClass);
        if (listeners == null)
            return 0;
    ...
    private <C extends Event> Consumer<C>
    reduce(Class<C> eventClass,
           Consumer<C> identity,
           Stream<Consumer<? extends Event>> consumers) {
